...
def endCorrect(lines):
    if(lines[-1].isdigit() and lines[-2].isdigit() and lines[-3].isdigit()):
        return True
    else:
        return False
...
def move_bb(bbs, t):
    ...
    BB : 2x4xn # e.g. n = 1 [[[x1] x2 x3 x4] , [y1 y2 y3 y4]] , e.g. n = 2 [[[x11 x12] [x21 x22] x3 x4] , [y1 y2 y3 y4]]
    T  : 2-long np.array
    """
    return bbs + t[:,None,None]

#pad = 5
def crop_safe(arr, rect, bbs=[], pad=0):# crop arr with rect xywh, nx4 xywh bbs just updated , with padding pad percent area
    """
    ARR : arr to crop
    ...
    """
    rect = np.array(rect)# convert rect xywh to array
    rect[:2] -= pad# x' = x-p, y' = y-p
    rect[2:] += 2*pad#w' = w+2p,h' = h + 2p
    v0 = [max(0,rect[0]), max(0,rect[1])]#x'>0 y'>0
    v1 = [min(arr.shape[0], rect[0]+rect[2]), min(arr.shape[1], rect[1]+rect[3])]# if arr width is small take it as end pt in x axis else x'+w' , ||ly y'+h'
    arr = arr[v0[0]:v1[0],v0[1]:v1[1],...]#x'/0 to arr_w/x'+w', ROHIT whats ,... ?
    if len(bbs) > 0:
...
class BaselineState(object):# return functions for ax^2 and 2ax
...
class RenderFont(object):
    ...
    def __init__(self, data_dir='data'):
        ...
        self.min_nchar = 2#2 MAYUR
        self.min_font_h = 8 #8 16 #changed by ayush earlier 16 #px : 0.6*12 ~ 7px <= actual minimum height
        self.max_font_h = 30 #120 #30 #px
        self.p_flat = 0.10
        ...
        # print("font state is --->" , self.font_state.sample())
        self.devchars= 'ाीिुूेैंोौृःॄॅअआइईउऊऋएऐओऔअंअःकखगघङचछजझञटठडढणतथदधनपफबभमयरलवषशसहक्षत्रज्ञड़ढ़।॥'
        pygame.init()
        # print("after init")
    ...
    def render_multiline(self,font,text):
        ...
        lines = text.split('\n')
        # print("render multiline is ->", text)
        ...
        x, y = 0, 0
        devShirorekhaSeparator = 0.9
        if (np.random.rand() < 0.1): devShirorekhaSeparator = 1
        for l in lines:
            x = 0 # carriage-return
            y += line_spacing # line-feed
            l = l.replace("\\s" + "ः","ः")
            devFlag = self.check_word_scriptDev(l)
            #print("devFlag", devFlag)
            if (devFlag):
                #print("l", l)
                l_new = self.aksharaSplit(l)
                #print("l_new", l_new)
                for ch in l_new: # render each character
                    if ch.isspace(): # just shift
                        ...
                        x += devShirorekhaSeparator*ch_bounds.width
    ...
    def render_curved(self, font, word_text):
        ...
        # create the surface:
        #word_text = re.split('\\s| ',word_text)[0]
        #word_text = word_text.split()[0]+word_text.split()[0]+word_text.split()[0]
        #print("word_text", word_text)
        devFlag = self.check_word_scriptDev(word_text)
        #print("devFlag", devFlag)
        if (devFlag):
            print("word_text", word_text)
            word_text_new = self.aksharaSplit(word_text)
            print("word_text_new", word_text_new)
            #word_text = word_text_new;
            wl = len(word_text_new)
            # print("word_text, word_text_new, wl", word_text, word_text_new, wl )
            # create the surface:
            ...
            for angleindx in range(len(rots)):
                rots[angleindx] = int(rots[angleindx]/len(word_text_new[angleindx])) 
            bbs = []
            # place middle char
            rect = font.get_rect(word_text_new[mid_idx])
            rect.centerx = surf.get_rect().centerx
            rect.centery = surf.get_rect().centery + rect.height
            rect.centery +=  curve[mid_idx]
            ch_bounds = font.render_to(surf, rect, word_text_new[mid_idx], rotation=rots[mid_idx])
            ...
            for i in range(wl):
                ...
                if i > mid_idx:
                    newrect.topleft = (last_rect.topright[0]-4, newrect.topleft[1])
                else:
                    newrect.topright = (last_rect.topleft[0]+4, newrect.topleft[1])
            ...
            bbs = []
            # place middle char
            rect = font.get_rect(word_text[mid_idx])#word_text
            rect.centerx = surf.get_rect().centerx
            rect.centery = surf.get_rect().centery + rect.height
            rect.centery +=  curve[mid_idx]
            ch_bounds = font.render_to(surf, rect, word_text[mid_idx], rotation=rots[mid_idx])#word_text
            ch_bounds.x = rect.x + ch_bounds.x
            ch_bounds.y = rect.y - ch_bounds.y
            mid_ch_bb = np.array(ch_bounds)
        ...
        '''print("Curved word to be printed",word_text, wl)
        lspace = font.get_sized_height() + 1
        lbound = font.get_rect(word_text)
        fsize = (round(2.0*lbound.width), round(3*lspace))
        surf = pygame.Surface(fsize, pygame.locals.SRCALPHA, 32)
    ...
    def get_nline_nchar(self,mask_size,font_height,font_width):# returns max no of lines and chars which can fit in MASK_SIZED image
        """
        Returns the maximum number of lines and characters which can fit
        in the MASK_SIZED image.
        """
        H,W = mask_size
        nline = int(np.ceil(H/(1.25*font_height)))
    ...
    def sample_font_height_px(self,h_min,h_max):
        if np.random.rand() < self.p_flat:
            ...
            rnd = np.random.beta(1.0, 3.0) #Added by ayush
    ...
    def render_sample(self,font,mask):# calling random_curved which is calling random_multiline
        ...
        """
        #H,W = mask.shape
        #print(mask,(~mask),(~mask).astype('float')/255)
...
class FontState(object):
    ...
    def get_aspect_ratio(self, font, size=None):
        ...
        """
        if size is None:
            size = 5 # doesn't matter as we take the RATIO
        ...
        try:
            ...
            # print(size)
            # print(chars)
            sizes = font.get_metrics(chars,size)#(min_x, max_x, min_y, max_y, horizontal_advance_x, horizontal_advance_y)
            # print("sizes is ",  sizes)
            good_idx = [i for i in range(len(sizes)) if sizes[i] is not None]
            # print("good_idx is ",  good_idx)
...
#Usage: self.text_source = TextSource(min_nchar=self.min_nchar = 2, fn=osp.join(data_dir,'newsgroup/newsgroup.txt'))
class TextSource(object):
    ...
    def __init__(self, min_nchar, fn):
        ...
        with codecs.open(fn, 'r', "utf-8") as f:
            self.txt = []
            for line in f:
                self.txt.append(line.strip())# appending each line(after removing \n in end) in self.txt
        
        # distribution over line/words for LINE/PARA:
        self.p_line_nline = np.array([0.85, 0.10, 0.05])#[::-1])
        self.p_line_nword = [4,3,8]#12]  # normal: (mu, std)
    ...
    def check_symb_frac(self, txt, f=0.35):#called in is_good, txt has all lines in file as each element, this checks = True if fraction of non-alphabetic/numeric text <= 0.35
        """
    ...
    def is_good(self, txt, f=0.35):# T if lines in text are valid, line l is valid when non-empty, symbol_fraction > f, has atleast 2(min_nchar) chars, and not all chars are i,x,0,O,-
        """
        ...
        def is_txt(l):
            ...
            return not np.all(chs)# np.all([1,1,1]) = True else false for all cases, so return 0 if all chars are i,x,0,O,- else return 1
    ...
    def center_align(self, lines): # say maxlen  = 29, and if a line has len 12 it adds 8 on left and 9 on right to make the line equisized to lin of maxlen
        """
        ...
        for i in range(len(lines)):
            l = lines[i].strip()
            dl = max_l-ls[i]# say 29 - 12 = 17 so 8 on left side and 9 on right side
            lspace = dl//2# 9
            rspace = dl-lspace# 18 - 9
            lines[i] = ' '*lspace+l+' '*rspace
        return lines

    def get_lines(self, nline, nword, nchar_max, f=0.35, niter=100):#get_lines(nline, nword, nchar_max, f=0.35) from para
        ...
        while not np.all(self.is_good(lines,f)) and iter < niter:
            ...
            # get words per line:
            nline = len(lines)
            # print("nline, lines is 738 ",  nline ,lines)
            for i in range(nline):
                words = lines[i].split()
                dw = len(words)-nword[i]
                if dw > 0:# chop off extra words
                    first_word_index = 0#random.choice(range(dw+1))
                    lines[i] = ' '.join(words[first_word_index:first_word_index+nword[i]])
                    # shift remaining words to next line or create a new line if we are at last line
                    if i < len(lines)-1:
                        lines[i+1] = ' '.join(words[first_word_index+nword[i]:]) + ' ' + lines[i+1]
                    #else:# create a new line if we are at last line
                        #lines.append(' '.join(words[first_word_index+nword[i]+1:]))
                        #nline = len(lines)
                #print("lines is below words ", lines)
                while len(lines[i]) > nchar_max: #chop-off characters from end:
                    toShiftToNextLine = ''
                    if not np.any([ch.isspace() for ch in lines[i]]):# if line is a word delete it as if we move it to next line next line cannot be taken
                        lines[i] = ''# just delete it
                    else:
                        toShiftToNextLine = lines[i][len(lines[i])-lines[i][::-1].find(' '):].strip() + ' ' + toShiftToNextLine
                        lines[i] = lines[i][:len(lines[i])-lines[i][::-1].find(' ')].strip()
                    if i < len(lines)-1:
                        #print("toShiftToNextLine", toShiftToNextLine)
                        lines[i+1] = toShiftToNextLine + lines[i+1].strip()
                    #else:# create a new line if we are at last line
                            #lines.append(lines[i][len(lines[i])-lines[i][::-1].find(' '):])
                            #nline = len(lines)
                    #print("lines is below lines ", lines)
        # print("lines is 762 ", lines)
...
#self.fdict = {'WORD':self.sample_word, 'LINE':self.sample_line, 'PARA':self.sample_para}
        
    def sample_word(self,nline_max,nchar_max,niter=100):
        rand_line = self.txt[np.random.choice(len(self.txt))]# sample a random line           
        words = rand_line.split()
        rand_word = random.choice(words)# choose a random word
