...
class TextRegions(object):
    ...
    def filter(seg,area,label):
        ...
        aidx = np.argsort(-area)#range(0,len(area))# more area first less area last
        #aidx = aidx[0:min(2,len(aidx)-1)]# ROHIT ADDED THIS LINE TO LIMIT NO. OF REGIONS
        good = good[filt][aidx]
        R = [R[i] for i in aidx]
...
def get_text_placement_mask(xyz,mask,plane,pad=2,viz=False):
    ...
    #_,contour,hier = cv2.findContours(mask.copy().astype('uint8'),
                                    #mode=cv2.RETR_CCOMP,
                                   # method=cv2.CHAIN_APPROX_SIMPLE) ----------MAYUR
...
def viz_textbb(fignum,text_im, bb_list,alpha=1.0):
    ...
    for i in range(len(bb_list)):
        ...
        for j in range(ni):
            bb = bbs[:,:,j]
            bb = np.c_[bb,bb[:,0]]
            plt.plot(bb[0,:], bb[1,:], 'r', linewidth=2, alpha=alpha)#plt.plot(bb[0,:], bb[1,:], 'r', linewidth=0, alpha=alpha) MAYUR
    plt.gca().set_xlim([0,W-1])
    plt.gca().set_ylim([H-1,0])
    plt.show(block=False)
def viz_textbbchar(fignum,text_im, bb_list,alpha=1.0):
    """
    text_im : image containing text
    bb_list : list of 2x4xn_i boundinb-box matrices
    """
    ...
    for i in range(len(bb_list)):
        ...
        for j in range(ni):
            bb = bbs[:,:,j]
            bb = np.c_[bb,bb[:,0]]
            plt.plot(bb[0,:], bb[1,:], 'b', linewidth=2, alpha=alpha)#plt.plot(bb[0,:], bb[1,:], 'r', linewidth=0, alpha=alpha) MAYUR
    plt.gca().set_xlim([0,W-1])
    plt.gca().set_ylim([H-1,0])
    plt.show(block=False)
